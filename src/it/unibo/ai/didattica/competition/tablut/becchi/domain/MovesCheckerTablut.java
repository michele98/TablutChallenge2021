...
public class MovesCheckerTablut implements MovesChecker {
    ...
    private boolean checkObstacles(int origin[], int destination[], State state){
        ...
        // non possono andare addosso alle altre nere, bianche, re o sul trono
    ...
    public HashSet<Action> getAllMoves(State state, State.Turn turnColor) throws IOException {
        ...
        if (turnColor.equals(StateTablut.Turn.WHITE) ) {
            int[] buf;
        ...
        } else if (turnColor.equals(StateTablut.Turn.BLACK)) {
            int[] buf;
            for (int i = 0; i < state.getBoard().length; i++) {
                for (int j = 0; j < state.getBoard().length; j++) {
    ...
    public HashSet<int[]> getAllMovesInt(State state, State.Turn turnColor) throws IOException {
        ...
        for (int[] i : movablePawns) {
            allMoves.addAll(obtainLegalMovesInt(i, state));
        }
        return  allMoves;
    }
